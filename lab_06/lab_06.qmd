...
```
#Professor Wells: 
What does this mean? Under line 262. I can tell it is warning me something but I am not quite sure what it is asking me to change. 
...
arrests_bar <- arrest_chart %>%
  select(year, disposition) %>%
  group_by(year) %>%
  count(disposition)

ggplot(dispo, aes(year, n, fill = "disposition=ARR")) +
  geom_col(position = "dodge") +
labs(
  title = "Arrests by Year",
  x = "Year",
  y = "Number of Arrests",
  caption = "Data source: SF"
...
top_12_dispo <- all_dispo %>%
  ...
  geom_text(aes(label=n), hjust = -.1, size = 2.5) +
    labs(
    title="popular 311 calls",
    x = "Outcomes",
    y = "Number",
    caption = "source: SF Call Data, from 2016-2019")
```
