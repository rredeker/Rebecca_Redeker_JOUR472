...
```{r}
maryland_tracts2 <- rio::import("maryland_tracts.xls")

maryland_counties_geoid2 <- rio :: import("md_counties_geoids.txt")

md_smith_data <- maryland_counties_geoid2 %>% 
...
The county with the highest average census response rate is Howard County, and the lowest rate was Worcester County. 
...
I believe that it wont work, because one category is named "Kent County". However, the other category is named "Kent County, MD". So, you cannot interjoin columns if they are not the same name, even if they measure to the same data. 
